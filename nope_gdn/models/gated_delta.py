...
class GatedDeltaLayer(nn.Module):
    """Gated Delta Network layer — official KDA parameterization.

    channel_wise_decay=True : log-space channel-wise decay
        g = -exp(A_log) * softplus(f_proj(x) + dt_bias)  (always <= 0)
    channel_wise_decay=False: scalar alpha per head (original GDN, ablation)

    Position info enters via the causal short-conv, the decay gate and the
    recurrent D x D state. forward_chunk() carries (state, conv_state) for
    streaming inference.
    """
    def __init__(self, hidden_size: int, num_heads: int = 4,
                 head_dim: int = 128, chunk_size: int = 64,
                 channel_wise_decay: bool = True,
                 decay_low_rank: int = None,
        ...
        if channel_wise_decay:
            # KDA: channel-wise log-space decay (low-rank f_proj)
            ...
            # decay_target_dt -> dt_bias = inv_softplus(target_dt): long-memory init
            if decay_target_dt is None:
            ...
            if decay_target_dt is not None:
                # shrink f_proj so target dt controls the initial decay
                nn.init.normal_(self.f_proj[-1].weight, std=1e-3)
        else:
            # Original GDN: scalar alpha per head
        ...
        self.g_proj = nn.Sequential(
            ...
            nn.Linear(head_dim, total_dim, bias=True),
        )
    ...
    def _short_conv(self, q, k, v, L, conv_state=None):
        """Depthwise causal convs with optional conv-state carry for streaming."""
    ...
    def forward_chunk(self, x, state=None, conv_state=None, use_parallel=None):
        """Streaming step: carries recurrent state and short-conv state."""
    ...
    def _core(self, x, state, conv_state, use_parallel):
        ...
        if self.allow_neg_eigval:
            beta = beta * 2.0  # allows eigenvalue -1 in Householder
        ...
        output = output.view(B, L, self.num_heads, self.head_dim)
        gate = self.g_proj(x).view(B, L, self.num_heads, self.head_dim)
    ...
    def _sequential(self, q, k, v, gate_or_alpha, beta, state, mode='kda'):
        """Step-by-step recurrence. kda: channel-wise exp(g); gdn: scalar alpha."""
        B, L, H, D = q.shape
    ...
    def _chunkwise(self, q, k, v, alpha, beta, state):
        """Chunkwise WY algorithm (scalar decay only)."""
        B, L, H, D = q.shape
        C = self.chunk_size
    ...
    def _chunkwise_channelwise(self, q, k, v, g, beta, state):
        """Chunkwise KDA — FLA fused Triton kernel when available, else loop."""
        B, L, H, D = q.shape

        if FLA_AVAILABLE:
            # q already L2-normed + scaled -> scale=1.0; uniform dtype for Triton
        ...
        # Fallback: Python loop in float32 (fp16 overflows DxD states)
        C = self.chunk_size
    ...
    def _process_chunk(self, q, k, v, alpha, beta, state):
        """Single-chunk WY core with log-space decay (scalar alpha only)."""
        B, H, C, D = q.shape
...
class BiGatedDeltaLayer(nn.Module):
    ...
    Wraps two GatedDeltaLayer instances; outputs are summed (Vim/VideoMamba
    convention, zero extra params). Pass num_heads = total_heads // 2 for
    parameter parity with a unidirectional layer.

    flip_mode: 'temporal' (reverse T only, default) | 'spatial' (reverse S
    only, ablation) | 'flat' (reverse whole flattened sequence, Vim-style).
    Streaming is NOT supported (backward branch needs future tokens).
    """
    ...
    def _flip_seq(self, x: torch.Tensor) -> torch.Tensor:
        """Reverse the sequence per flip_mode. Sequence layout is row-major
        (T', H'*W'): 'temporal' reverses T only, 'spatial' reverses S only,
        'flat' reverses the whole flattened sequence."""
        B, L, D = x.shape
    ...
    def forward(self, x: torch.Tensor, state=None, use_parallel=None):
        """
        Returns (y_fwd + y_bwd_realigned, None) — state is ignored in bidi mode.
        """
