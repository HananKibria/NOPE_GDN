from main import dataclass,field
...
class ModelConfig:
    ...
    #Processor (Stage 2) - only used for NOPE_GDN variants
    ...
    gdn_temporal_only: bool = True #GDN scans time-only per spatial location (vs flat T'xS raster scan)
    decay_target_dt :float = 0.05
    a_init_range: Tuple[float, float] = (0.5, 0.8)
...
def get_Config(variant: str="nope_gdn", size: str ="small") -> FullConfig:
    """Returns a FullConfig object based on the specified model variant and size."""
    cfg = FullConfig()
